          ...
          <el-row :gutter="10">
            <el-col :span="12" >
                <el-form-item prop="branchId" label="所属机构：">
                    <select-tree :checked-ids="newUserDate.branchId" v-model="newUserDate.branchId" @selectCheck="getIds" style="min-width:170px;"
                :props='{id:"branchId",label: "branchName",children: "childBranch"}' :tree-data="brno" :node-key="'branchId'" ></select-tree>
                <!-- <el-input v-model="newUserDate.BranchNo"
...
export default {
  ...
  methods: {
    initList(){
      ...
      this.checkedIds[0] = this.optionUpdate.currentRow.branchId; //回显机构号
    },
    openUpdate() {
      this.initList()
      for(var key in this.optionUpdate.currentRow){
        if (key=="branchId") {
