<template>
  <div class="meeting-msg-query" v-loading="loading" element-loading-text="加载中。。。">
    <!-- 主页面 -->
    <div>
       <el-card>
       ...
       <el-card style="margin:10px 0 0 0">
          <el-row >
              <el-col :md="12" :lg="12">
                <el-button type='danger' size='mini' @click="closeMeeting" :disabled="!(selectList.length==1&&selectList[0].STATUS==1)">终止会议</el-button>
              </el-col>
          </el-row>
          <el-table 
          ref="tableData" 
    ...
    <el-dialog :title="'详细信息'" :visible.sync="detailPopStatu" :lock-scroll="false" width="800px">
      ...
      <div slot="footer" class="dialog-footer">
        <!-- <el-button  @click="configPopStatu=false" size="mini">取 消</el-button> -->
        <el-button type="primary" size="mini" @click="detailPopStatu=false">确 定</el-button>
      </div>
    </el-dialog>
...
export default {
  name: "meeting-msg-query",
  // components: { selectTree },
  data() {
    return {
      loading: false,
      selectList: [],
      meetingStatu: [{value:'',label:''},{value:'0',label:'未开始'},{value:'1',label:'正常'},{value:'2',label:'结束'},{value:'3',label:'异常'}],
      // brno: [],
      tableData: [],
      currentPage: 1,
  ...
  methods: {
    ...
    meetingStatuFormat(row,column){
      ...
      var temp=this.toolBox.objArray2Hash(this.meetingStatu,'value');
      // console.log(row,column,this.tableData);
    ...
    closeMeeting(){
      // 确定是否关闭
      this.$confirm('此操作将关闭'+this.selectList[0].CONFERID+'会议, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.queryCloseMeeting();
      }).catch(() => {
        // this.$message({
        //   type: 'info',
        //   message: '已取消删除'
