          ...
          <el-col :span="24">
            <el-card style="margin:0 0 0 10px;">
                <el-form ref='queryForm' :model='queryForm'  size='mini' label-position="top">
                  <el-row :gutter="10">
                      <el-col :md="7" :lg="{span:8,offset:16}" style="text-align:right;">
                          <el-tooltip class="item" content="监控配置" placement="bottom-end">
                            <el-button type="" :icon="'el-icon-setting'" style="margin: 9px auto;" @click="openConfigPop"></el-button>
                          </el-tooltip>
                          <el-tooltip class="item" content="全屏显示" placement="bottom-end">
                            <el-button type="" :icon="'el-icon-rank'" style="margin: 9px auto;"></el-button>
                          </el-tooltip>
                          <el-tooltip class="item" :content="showMatrix?'列表模式':'矩阵模式'" placement="bottom-end">
      ...
      <el-dialog title="监控配置" :visible.sync="configPopStatu" :lock-scroll=false width="600px">
            <el-card>
              <el-form ref='queryFormConfig' :model='queryFormConfig'  size='mini' label-position="top">
                <el-row :gutter="10">
                  ...
                  <el-col :md="7" :lg="12">
                        <el-form-item label='机构编号' prop='brno'>
                          <select-tree ref="tree" :checked-ids="checkedIds" v-model="queryFormConfig.brno" 
                  ...
                  <el-col :md="7" :lg="12">
                        <el-form-item label='设备状态'  prop='devStatu'>
                            <el-cascader v-model='queryFormConfig.devStatu' :props="{value: 'devState',label: 'devStateName'}"
                ...
                <div style="border-top: 1px solid #ccc;margin:15px 22px 12px;"></div>
                <el-row>
                  <el-col :md="24">
                    <h4>轮询配置</h4>
                  </el-col>
                  <el-col :md="7" :lg="12">
                    <el-form-item label='轮询间隔（秒）' prop=''>
                      <el-input v-model="queryTimeSpaceConfig" placeholder="请输入轮询间隔"></el-input>
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="configSubmit" size="mini">确 定</el-button>
        </div>
      </el-dialog>
      <!-- 远程管理弹窗 -->
...
export default {
  ...
  methods: {
    ...
    openConfigPop(){
      ...
      this.queryTimeSpaceConfig=(this.queryTimeSpace/1000).toFixed(2);
    ...
    configSubmit(){
      ...
      this.queryTimeSpace=this.queryTimeSpaceConfig*1000;
      this.queryModel();
  ...
  computed: {
    ...
    queryTimeSpaceShow(){//显示秒数更改轮询配置    弹窗时暂停轮询
