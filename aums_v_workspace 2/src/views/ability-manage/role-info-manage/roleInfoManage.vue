<template>
  <div   v-loading="loading">
    ...
    <el-row :gutter="10">
      ...
      <el-col :lg="16" :md="24">
        ...
        <el-card style="overflow:visible; height:688px">
            <el-col :span="24">
              ...
              <div class="roleBox">
                <el-button style="margin-left:10px" type="primary" icon='el-icon-plus' size="mini" :disabled="isDisabledAdd" @click="dialogOptionsAdd.isShow=true;">新增</el-button>
                <el-button style="margin-left:10px" type="danger" icon='el-icon-delete' size="mini" :disabled="isDisabledDel" @click="deleteRole">删除</el-button>
                <el-button style="margin-left:10px" type="primary" icon='el-icon-edit' size="mini" :disabled="isDisabledDel" @click="dialogOptionsEdit.isShow=true">修改</el-button>
                <el-button style="margin-left:10px" type="primary" icon='el-icon-search' size="mini" :disabled="isDisabledDetail" @click="openRoleDetail">交割能力明细</el-button>
      <el-table
        :data="roleSummaryInfo"
        ...
        <el-table-column width="60">
          <template slot-scope="scope">
            ...
            v-model="ABILITYID">{{null}}</el-radio>
          </template>
        </el-table-column>  
        <el-table-column
            prop="ABILITYNAME" label="交割能力名称" >
        </el-table-column>
        <el-table-column
            prop="ISITOMNT" label="是否全能" :formatter="ISITOMNTFormat" >
        </el-table-column>
        <el-table-column 
       align="center"
         label="详细信息" fixed="right">
          <template slot-scope="scope">
            <el-button type="text" size="mini"
            :disabled="scope.row.ISITOMNT === '1'"
      ...
      <!-- 弹窗 -->
    <keep-alive>
      <component :is='dialogList.updateRoleDetail' :option='dialogOptionsUpdateDetail' :rowData="rowVal"  @refreshTable="initQueryAbility"></component>
    </keep-alive>
    <keep-alive>
      <component :is='dialogList.updateRole' :option='dialogOptionsUpdate' :rowData="rowVal"  @refreshTable="initQueryAbility"></component>
    </keep-alive>
    <component :is='dialogList.createRole' :option='dialogOptionsAdd' :rowData="rowValAdd" @refreshTable="initQueryAbility"></component>  
    <component :is='dialogList.editRole' :option='dialogOptionsEdit' :rowData="rowVal" @refreshTable="initQueryAbility"></component>          
  </div>
</template>
<script>
...
import updateRoleDetail from "./dialog/updateRoleDetail";
...
export default {
  ...
  methods: {
    ...
    roleChange(val) {
      this.isDisabledDel = false;
      this.roleSummaryInfo = val;
      console.log("当前选择交割能力概要信息：");
      console.log(this.roleSummaryInfo);
      this.queryAbilityDetail();
      // this.roleId=val.ABILITYID;//暂存当前ID
      //传递值给子组件
      this.dialogOptionsUpdate.currentRowList=val;
      this.dialogOptionsUpdateDetail.currentRowList=val;
    },
    ...
    //点击物料时触发：
    roleMatlChange(val) {
      this.roleDetailInfo = val;
      console.log("当前选择交割能力物料信息：");
      console.log(this.roleDetailInfo);
      //过滤
      this.fillterValue(this.roleDetailInfo.DELIVERYMATLTYPE);
      this.fillterRosValue(this.roleDetailInfo.MATLPROS);
      // this.roleId=val.ABILITYID;//暂存当前ID
      //传递值给子组件
      this.dialogOptionsUpdate.currentRowList=val;
      this.dialogOptionsUpdateDetail.currentRowList=val;
    ...
    queryAbilityDetail(){
      // 此处模拟数据，后期通信传入数据
      this.roleMatlCheckArray = Object.assign([]);
      let resBody = new deliverAbilityDetailQuery();
      resBody.data.ABILITYID = this.roleSummaryInfo.ABILITYID;
      resBody.data.pageflag = this.pageflag;
      resBody.data.currpage = this.currpage;
      resBody.data.pagerownum = this.pagerownum;
      request(resBody)
      .then(response=>{
        if(response.SYS_HEAD.ReturnCode === "000000") {
          ...
          console.log(list);
          for (let i = 0; i < list.length; i++) {
                this.roleMatlCheckArray.push(list[i]);
              }
            console.log("可选择项数组：");
            console.log(this.roleMatlCheckArray);
          // this.roleDetailInfo = response.RSP_BODY.struct[0];
          console.log("当前交割能力明细信息：");
          console.log(this.roleDetailInfo);
        } else {
          console.log(response);
        }
    ...
    // 删除输出物料
    deleteRole() {
      this.$confirm('此操作将永久删除该交割能力,同时包含该能力的相关数据集（包括：可交割能力明细信息、网点用户交割能力）都将删除,是否继续?', '提示', {
          ...
          resBody.data.ORGNO = this.rowVal.ORGNO;
          resBody.data.ABILITYNAME = this.rowVal.ABILITYNAME;
          resBody.data.ABILITYID = this.rowVal.ABILITYID;
          resBody.data.ISITOMNT = this.rowVal.ISITOMNT;
