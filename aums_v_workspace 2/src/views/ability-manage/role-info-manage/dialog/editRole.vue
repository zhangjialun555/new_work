<template>
  <div>
    <el-dialog :visible.sync="isShow" title="编辑交割能力"
      @open="openAddRole" 
      @close='closeAddRole'
       width="600px"
      v-loading="loading"
    element-loading-text="加载中。。。">
      <el-steps finish-status="success" :active='active' align-center>
        <el-step title='交割能力'></el-step>
        <el-step v-if="roleManage.ISITOMNT===false" title='可交割物料'></el-step>   
        <el-step v-if="roleManage.ISITOMNT===false" title='明细维护'></el-step> 
      </el-steps>
      <div  v-if="active==0">
            ...
            <!-- <el-row class="rowStyle" >
              ...
              <el-row class="rowStyle" >
                <el-col :span="16">
                  ...
                  <el-input v-model="roleManage.ABILITYNAME" maxlength="50" placeholder="请输入能力名称"></el-input>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row class="rowStyle" :gutter="100">            
               <el-col :span="12" >
                    <el-form-item prop="ISITOMNT" label=" 是否全能">
                                    <el-switch v-model="ISITOMNT"></el-switch>
                                </el-form-item>
        ...
        <el-row style="margin:20px 0;">
          <el-col :offset="10" :span="2">
            <el-button v-if="ISITOMNT===true" type="primary" size="mini" @click="addRoleEnd">完成</el-button>
      ...
      <div   v-if="active==2">
        <el-card >
          <el-form ref="roleManageDetail" :model="roleManageDetail"  :rules="rulesRole"
            size="mini" label-width="120px" label-position="left">
              <el-row class="rowStyle" :gutter="100" v-if="matlTypeFlag === 'In'">            
               <el-col :span="12" >
...
export default {
  ...
  methods: {
    ...
    //初始化当前参数值：
    initRoleInfo(){
        this.roleManage = this.rowData;
        if(this.roleManage.ISITOMNT ==="1"){
            this.ISITOMNT = true;
        }else{
            this.ISITOMNT=false;
        }
        console.log("处理后的能力：");
        console.log(this.roleManage);
    },
    // 查看当前行参数详情信息
    ...
    queryDeliverMatl(){
      ...
      .then(response=>{
        if(response.SYS_HEAD.ReturnCode === "000000") {
          this.roleManageDetail = response.RSP_BODY.struct;
    ...
    addRoleEnd(){
      this.$refs["roleManage"].validate(valid => {
        if (valid) {
          ...
          resBody.data.ABILITYNAME = this.roleManage.ABILITYNAME;
          resBody.data.ABILITYID = this.roleManage.ABILITYID;
          resBody.data.ISITOMNT = isItOmnt;
          resBody.data.MODUSER = this.MODUSER;
          resBody.data.MODORGNO = this.MODORGNO;
          request(resBody)
          .then(response=>{
    ...
    next(){
      this.active = 2;
      if(this.roleManageDetail.MATLPROS === '01'){
        this.matlTypeFlag = 'In'
      }else {
        this.matlTypeFlag = 'Out'
      }
    },
    ...
    roleDetail() {
      ...
      resBody.data.MODORGNO = this.MODORGNO;
       request(resBody)
        ...
        // if (this.roleManageDetail) {
        //   console.log(this.roleManageDetail)
        //   this.$message({
        //     type: "success",
        //     message: "角色关联成功!"
        //   });
